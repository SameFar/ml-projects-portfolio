...
CATEGORY_MAP = {
    ...
    "eye_colour": np.array(
        ["Green", "Blue", "Hazel", "Grey", "Black", "Light Brown", "Light Blue"]
    ),
...
OUTPUT_MAP = {
    ...
    "Recommended Jewelry Metal": np.array(["Gold", "Silver", "Rose Gold"]),
...
def convert(v: FashionInput) -> np.ndarray:
    return np.concatenate(
        [(cats == getattr(v, attr)).astype(int) for attr, cats in CATEGORY_MAP.items()]
    )
