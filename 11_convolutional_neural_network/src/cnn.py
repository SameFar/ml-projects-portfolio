...
class CompleteCNN:
    ...
    def forward(self, X, is_training=True):
        ...
        for i in range(self.depth):
            for j in range(self.in_depth):
                self.conv_out[i] += signal.correlate2d(
                    self.X[j], self.kernals[i, j], "valid"
                )
        ...
        if is_training:
            self.dropout_mask = (
                np.random.rand(*self.flat_out.shape) >= self.dropout_rate
            )
            self.flat_out = (self.flat_out * self.dropout_mask) / (
    ...
    def backward(self, target_label):
        ...
        dense_input_gradient = (dense_input_gradient * self.dropout_mask) / (
            1.0 - self.dropout_rate
        )
        ...
        for i in range(self.depth):
            for j in range(self.in_depth):
                kernal_gradient[i, j] = signal.correlate2d(
                    self.X[j], conv_gradient[i], "valid"
                )
                input_gradient[j] += signal.correlate2d(
                    conv_gradient[i], self.kernals[i, j], "full"
                )
