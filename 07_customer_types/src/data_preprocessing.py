...
def get_clean_data(initial_df):
    ...
    df['Total Refund Recived'].fillna(0, inplace=True)
    df['Avg Spent'] = (df['Total Spent'] / df['Transactions']).round(2)
    df['Purchases per month'] = (df['Transactions'] / 12).round(2)
    df['Return Rate'] = customers['Customer ID'].count() // refunds.groupby('Customer ID')['Total'].count()
    df['Return Rate'].fillna(0, inplace=True)
