...
app = FastAPI(title="Customer Types", version="1.0.0")
...
async def prediction(cmr: CustomerYearlyMetrics):
    ...
    predicted_label = labels.get(
        prediction_index, f"Unknown key received from model: {prediction_index}"
    )

    return JSONResponse(
        status_code=200, content={"predicted_customer": predicted_label}
    )
