...
def train(model, dataloader, epochs=20, lr=0.0001):
    ...
    with torch.no_grad():
        ...
        logging.info(
            f"Post-training embedding average standard deviation: {emb_std:.6f}"
        )
        if emb_std < 1e-4:
            logging.warning(
                "⚠️ CRITICAL WARNING: Standard deviation is near zero! The model has likely suffered Hypersphere Collapse."
            )
        ...
        logging.info(
            f"Derived 90th percentile prediction threshold cutoff: {dynamic_threshold:.6f}"
        )
