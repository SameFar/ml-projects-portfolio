...
def predict_preferences(model, dataloader, threshold: float = None):
    ...
    if center is None:
        raise ValueError("The provided model instance hasn't been calibrated with an SVDD center yet.")
    ...
    logging.info(f"Evaluating dataloader tracks on native device: {model_device} (Threshold: {threshold:.4f})...")
        ...
        for i, batch in enumerate(dataloader):
            ...
            for idx, dist_tensor in enumerate(distances):
                ...
                predictions.append({
                    "batch_idx": i,
                    "sample_idx": idx,
                    "distance_score": round(distance_score, 6), # Increased precision for small variance tracking
                    "verdict": verdict,
