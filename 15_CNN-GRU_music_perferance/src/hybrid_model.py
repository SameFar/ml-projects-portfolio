...
DEFAULT_CHECKPOINT_PATH = Path(__file__).resolve().parent.parent / 'model' / 'Cnn14_emb512_mAP=0.420.pth'

class PannGru(nn.Module):
    def __init__(self, hidden_size: int = 128, checkpoint_path: Path = DEFAULT_CHECKPOINT_PATH):
    ...
    def forward(self, waveform: torch.Tensor, chunk_duration_samples: int = 80000) -> torch.Tensor:
        """
        Args:
