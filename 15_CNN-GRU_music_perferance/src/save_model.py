...
def save_model(model, filename: str = "pann_gru.pth", model_dir: Path = MODEL_DIR):
    ...
    if center is None:
        logging.warning("The model does not have a 'center' attribute computed. Saving weights only.")
    ...
    logging.info(f"Successfully saved one-class model and center cluster to: {save_path}")
...
def load_model(model, filename: str = "pann_gru.pth", model_dir: Path = MODEL_DIR):
    ...
    if model.center is None:
        logging.warning("No 'center' vector was found in this checkpoint. Inference will require re-centering.")
    else:
