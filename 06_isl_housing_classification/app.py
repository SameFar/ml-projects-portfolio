...
if not os.path.exists(MODEL_PATH):
    raise FileNotFoundError(
        f"Production model weight artifact not resolved at: {MODEL_PATH}. Run training pipeline first."
    )
...
class PropertyInferenceSchema(BaseModel):
    baths: int = Field(
        ..., description="Number of bathrooms", json_schema_extra={"example": 3}
    )
    beds: int = Field(
        ..., description="Number of bedrooms", json_schema_extra={"example": 4}
    )
    marla: float = Field(
        ...,
        description="Total land area computed in Marlas",
        json_schema_extra={"example": 10.0},
    )
    rooms: int = Field(
        ...,
        description="Count of extra premium rooms (Drawing/Powder/Steam/etc.)",
        json_schema_extra={"example": 1},
    )
    location: str = Field(
        ...,
        description="Specific sector or neighborhood name",
        json_schema_extra={"example": "G-13"},
    )
...
def predict_market_bracket(payload: PropertyInferenceSchema):
    try:
        ...
        return {"prediction_index": class_idx, "predicted_price_bracket": price_range}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Inference Engine Runtime Fault: {str(e)}"
        )
