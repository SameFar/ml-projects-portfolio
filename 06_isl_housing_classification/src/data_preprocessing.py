...
def load_and_clean_data(file_path):
    ...
    isl_df = raw_df[raw_df['Location'].str.contains('Islamabad', case=False, na=False)].copy()
    ...
    clean_df = isl_df[isl_df.groupby('Location')['Location'].transform('count') >= 25].copy()
    
    # Clean location strings and compile dummy variables
    clean_df['Location'] = clean_df['Location'].str.replace(', Islamabad, Islamabad Capital', '', regex=False)
    ...
    clean_df['Marla'] = area_vals.where(area_units == 'marla', area_vals * 20.0).astype(float)
    ...
    room_features = ['Steam Room', 'Prayer Rooms', 'Dining Room', 'Laundry Room', 'Drawing Room', 'Lounge or Sitting Room', 'Powder Room']
    clean_df['Other Rooms'] = clean_df[room_features].sum(axis=1)
    
    dropped_cols = room_features + ['Store Rooms', 'Kitchens', 'Gym', 'No additional rooms']
    clean_df.drop(columns=dropped_cols, inplace=True)
    
    # Discretize continuous pricing curves into target ordinal brackets
    bins = [0, 5, 10, 15, 20, np.inf]
    labels = [0, 1, 2, 3, 4]
    clean_df['price_category'] = pd.cut(clean_df['Crore'], bins=bins, labels=labels, right=False).astype(int)
...
def run_macro_stratified_split(df):
    ...
    def tag_macro_neighborhoods(row):
        if row.get('Bahria Town', 0) == 1: return 'Bahria'
        if row.get('G-13', 0) == 1: return 'G13'
        if row.get('DHA Defence', 0) == 1: return 'DHA'
        return 'Other'
        
    stratify_proxy = X.apply(tag_macro_neighborhoods, axis=1)
    return train_test_split(X, y, test_size=0.2, stratify=stratify_proxy, random_state=42)
