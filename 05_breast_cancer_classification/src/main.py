...
from models import get_outlier_detector, get_parametric_pipelines, get_non_parametric_models
...
def main():
    ...
    # Isolate outlier profiling to parametric training subset to eliminate test leakage
    ...
    # Group A: Fit geometric & probabilistic models using variance-stabilized, cleaned training data
    ...
    # Group B: Fit recursive partitioning tree models using the unmodified raw training distribution
    ...
    df_compare = pd.DataFrame(metrics_log).sort_values(by="F1-Score", ascending=False).reset_index(drop=True)
    
    # Establish filesystem archive locations
