...
if __name__ == "__main__":
    ...
    sample_data_path = os.path.join(base_dir, "../data/sample.csv")

    print("--- Loading Serialized Production Models ---")
    models = load_all_models(models_directory)

    print(
        f"--- Processing Inbound Sample from {os.path.basename(sample_data_path)} ---"
    )
    raw_data = pd.read_csv(sample_data_path)
    sample_row = raw_data.sample(
        1, random_state=42
    )  # Fixed seed for reproducible output checks
