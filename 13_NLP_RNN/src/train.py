...
def train(
    ...
    train_loader = DataLoader(
        training_data, batch_size=n_batch_size, shuffle=True, collate_fn=lambda x: x
    )

    val_loader = DataLoader(
        val_data, batch_size=n_batch_size, shuffle=False, collate_fn=lambda x: x
    )
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        with (
            torch.no_grad()
        ):  # Disables gradient calculation to save memory and compute
        ...
        if epoch % report_every == 0:
            logging.info(
                f"{epoch} ({epoch / n_epoch:.0%}): \t average batch loss = {all_losses[-1]:.4f} \t Val Accuracy = {val_accuracy:.2%}"
            )
        current_loss = 0
